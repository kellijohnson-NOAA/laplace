...
SS_testing = function(RepI, Version){
  ...
  file.copy(from=paste(DateFile,FileSet_From,sep=""), to=paste(RepFile,FileSet_To,sep=""), overwrite=TRUE)
  if(ReRun==TRUE | !("BootData.ss"%in%list.files(RepFile) & "ss3_0.par"%in%list.files(RepFile))){
    # Copy files
    STARTER = SS_readstarter(paste(DateFile,"Starter.SS",sep=""), verbose=FALSE)
    
    # Deal with randomization (i.e. its necessary to de-synchronize generating random values using ADMB and parallelized code)
    ...
    while(Converged==FALSE){
      # Change starter file
      Starter = SS_readstarter(paste(RepFile,"Starter.SS",sep=""))
